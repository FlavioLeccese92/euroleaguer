...
# Get data needed
CompetitionRounds = getCompetitionRounds("E2023") %>% filter(MinGameStartDate <= Sys.Date())

CompetitionStandings = getCompetitionStandings("E2023", max(CompetitionRounds$Round))
...
# Construct Court
source("vignettes/scripts/construct-court.R")
...
GamePoints = GamePoints %>%
  ...
  left_join(CompetitionStandings %>%
              select(TeamCode, TeamName, TeamImagesCrest, Position), by = "TeamCode") %>%
  arrange(Position) %>%
...
PlotTitle = glue("<span style = 'font-size: 32px'>Team Court goals and accuracy</span><br>
...
PlotSubtitle = glue(" <span><img src = 'man/figures/euroleague-logo-vertical.png' height='50'></span>")
...
e = GamePoints %>%
  ...
  geom_image(data = TeamImage, aes(x = x, y = y, image = TeamImagesCrest), size = 0.2,
  ...
  theme(
    ...
    plot.subtitle = element_markdown(hjust = 1, size = 16, margin = margin(-30, 3, -50, 0)),
